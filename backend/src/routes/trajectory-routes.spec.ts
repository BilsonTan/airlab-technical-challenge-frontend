...
describe('Trajectory Routes', () => {
  ...
  beforeEach(() => {
    ...
    (trajectoryService.getAllTrajectories as jest.Mock).mockReturnValue(
      mockTrajectories
    );
    (trajectoryService.getAllAirports as jest.Mock).mockReturnValue(
      mockAirports
    );
  });
  ...
  test('GET / should filter trajectories by time range', async () => {
    ...
    const response = await request(app).get(
      `/api/trajectories?startTime=${startTime}&endTime=${endTime}`
    );
  ...
  test('GET / should handle combined filters', async () => {
    const response = await request(app).get(
      '/api/trajectories?departure=EDDF&arrival=LFPG&startTime=2023-01-01T11:00:00Z'
    );
